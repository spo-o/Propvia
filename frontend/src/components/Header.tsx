...
interface HeaderProps {
  // Backend Logic Found: Notifications data coming from backend
...
export default function Header({
  ...
  const handleLogout = () => {
    // Backend Logic Found: Logout process (token/session clearing on backend)
    logout();
    ...
    <header className="bg-gradient-to-r from-brand via-brand-600 to-brand-700 shadow-xl backdrop-blur-md fixed top-0 left-0 right-0 z-[100] border-b border-brand-500/20">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2 lg:space-x-4">
            <>
              ...
              <Link
                to="/platform"
                className="hidden lg:inline-flex items-center px-6 py-3 bg-gradient-to-r from-accent to-accent-light text-brand-900 font-bold rounded-xl hover:from-accent-light hover:to-accent transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl"
              >
                Explore Properties
              </Link>
              {isAuthenticated && (
                  ...
                  <Tooltip.Provider>
                    <Tooltip.Root>
                      <Tooltip.Trigger asChild>
                        <Link
                          to={"/ask"}
                          className="hidden md:flex items-center space-x-2 px-5 py-3  backdrop-blur-sm text-white font-medium rounded-xl border border-white/20 hover:bg-white/10 transition-all duration-300"
                        >
                      ...
                      <Tooltip.Portal>
                        ...
                        >
                          <p className="text-sm text-gray-600 leading-relaxed">
                            Ask our AI about any property or neighborhood in
                            Detroit! Get instant insights about market trends,
              ...
              <div className="flex items-end justify-between lg:hidden z-[100]">
                <div
                  className={`lg:hidden fixed top-0 left-0 h-screen w-64 z-50 bg-gradient-to-r from-brand via-brand-600 to-brand-700 backdrop-blur-sm flex flex-col shadow-lg transition-transform duration-300 ease-in-out ${
                ...
                >
                  ...
                  <div className="flex flex-col gap-4 px-2 py-6">
                    {/* Analysis reports */}
                    <button
                      onClick={() => handleMobileLink("/platform")}
      ...
      <Dialog.Root open={showLogoutDialog} onOpenChange={setShowLogoutDialog}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-[50%] left-[50%] max-h-[85vh] w-[90vw] max-w-[450px] translate-x-[-50%] translate-y-[-50%] rounded-2xl bg-white/95 backdrop-blur-md p-8 shadow-2xl border border-gray-200 z-[201]">
            <div className="text-center">
              ...
              <Dialog.Title className="text-2xl font-bold mb-3 text-gray-900">
                Sign Out of Propvia?
              </Dialog.Title>
              ...
              <div className="flex gap-4 justify-center">
                <button
                  ...
                  className="px-6 py-3 text-gray-700 hover:bg-gray-100 rounded-xl transition-all duration-300 font-medium border border-gray-200 hover:border-gray-300"
                >
                  Cancel
                </button>
                <button
                  onClick={confirmLogout}
                  className="px-6 py-3 bg-gradient-to-r from-red-600 to-red-700 text-white rounded-xl hover:from-red-700 hover:to-red-800 transition-all duration-300 font-bold shadow-lg hover:shadow-xl transform hover:scale-105"
                >
                  Yes, Sign Out
                </button>
              </div>
